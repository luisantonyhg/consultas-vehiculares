...
import { renderPdfInto } from '../../ui/pdf_viewer.js';
...
function openDocumentModal(blob, filename) {
    ...
    modal.innerHTML = `<div class="flex h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div class="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3"><div><p class="text-xs font-black text-slate-900">Documento oficial SIGM</p><p class="text-[10px] text-slate-500">${safeFilename}</p></div><div class="flex gap-2"><a href="${objectUrl}" download="${safeFilename}" class="rounded-lg bg-lime-600 px-3 py-2 text-[10px] font-black text-white"><i class="fas fa-download mr-1"></i>Descargar</a><button type="button" data-close-sigm class="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-700"><i class="fas fa-xmark"></i></button></div></div><div data-pdf-pages class="min-h-0 flex-1 overflow-auto bg-slate-200 p-2"><p class="p-6 text-center text-sm text-slate-500">Preparando documento…</p></div></div>`;
    ...
    if (pages instanceof HTMLElement) {
        renderPdfInto(pages, blob).catch(() => {
            pages.innerHTML = '<p class="p-6 text-center text-sm text-slate-600">No se pudo mostrar una vista previa en este navegador. Usa Descargar para abrir el PDF.</p>';
        });
    }
}
